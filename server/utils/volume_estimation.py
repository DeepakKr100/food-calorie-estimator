...
def estimate_volumes_via_gpt4o(image_cv, debug=False):
    ...
    system_prompt = (
        ...
        "You see an image that includes a standard credit card for scale. "
        "The card measures 8.56 cm x 5.398 cm. Also in the image are one or more "
        "food items. Please do the following:\n\n"
        "1. Identify each distinct food item.\n"
        "2. Estimate the volume of each item in milliliters (mL), using the card as a reference.\n"
    ...
    user_content = [
        {
            "type": "text",
            "text": "Here is the image. Identify each food item and its volume in mL using the credit card as a reference scale."
        },
        {
            "type": "image_url",
            "image_url": {
